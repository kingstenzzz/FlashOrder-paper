...
FlashOrder is a practical high-performance fair ordering protocol. FlashOrder is motivated by the fact that fair ordering among two transactions is needed when both transactions access a shared resource.

# FlashOrder Installation

### Install from the repo
* git clone https://github.com/\<your-org\>/FlashOrder-paper.git
* cd FlashOrder-paper/
...
* on Ubuntu: sudo apt-get install libssl-dev libuv1-dev cmake make g++
...
Note: In this example, we use a typical Linux image, Ubuntu 18.04, on Cloudlab. But any machine with Ubuntu 18.04 installed may work, in general.
...
* Build `FlashOrder` on all remote machine by `./run.sh setup`.
...
* (for replicas) Create a new experiment run and start all replica processes by `./run.sh new myrun1`. (wait for a while until all replica processes settle down, for good network like EC2, 10 seconds should be more than enough)
