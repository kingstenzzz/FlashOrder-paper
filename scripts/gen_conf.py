...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate configuration file for a batch of FlashOrder replicas"
    )
    ...
    parser.add_argument("--keygen", type=str, default="./flashorder-keygen")
    parser.add_argument("--tls-keygen", type=str, default="./flashorder-tls-keygen")
    ...
    if args.ips is None:
        ...
        ips = [l.strip() for l in open(args.ips, "r").readlines() if l.strip()]
        print(f"DEBUG: Found {len(ips)} IPs in {args.ips}")
    prefix = args.prefix
    iter = args.iter
    ...
    p = subprocess.Popen(
        [keygen_bin, "--num", str(len(replicas))],
        stdout=subprocess.PIPE,
        stderr=open(os.devnull, "w"),
    )
    keys = [[t[4:] for t in l.decode("ascii").split()] for l in p.stdout]
    tls_p = subprocess.Popen(
        [tls_keygen_bin, "--num", str(len(replicas))],
        stdout=subprocess.PIPE,
        stderr=open(os.devnull, "w"),
    )
