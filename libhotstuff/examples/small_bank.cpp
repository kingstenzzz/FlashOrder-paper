...
void SmallBank::transaction_savings(uint64_t user_id, uint64_t amount) {
  if (UINT64_MAX - saving_accounts[user_id] < amount) {
    /* If amount is exceded to the maximum limit of uint64_t : discart
     * transaction */
    return;
  }
...
void SmallBank::write_check(uint64_t user_id, uint64_t amount) {
  if (checking_accounts[user_id] < amount) {
    /* If the user has not sufficient amount in the checking account : discart
     * transaction */
    return;
  }
  checking_accounts[user_id] -= amount;
...
void SmallBank::send_payment(uint64_t from_user_id, uint64_t to_user_id,
                             uint64_t amount) {
  if (checking_accounts[from_user_id] < amount ||
     ...
     */
    /* If to_user_id amount is exceded to the maximum limit of uint64_t :
     * discart transaction */
    return;
  }
...
void SmallBank::transaction_split(
...
SmallBankManager::SmallBankManager(uint64_t n_users, double prob_choose_mtx,
  ...
  mtx_distribution = std::uniform_int_distribution<uint64_t>(random_mtx.first,
                                                             random_mtx.second);

  random_users = std::make_pair(0, n_users - 1);
  user_distribution = zipfian_int_distribution<uint64_t>(
      random_users.first, random_users.second, skew_factor);
  ...
  //     mtx_distribution(mtx_generator));
  // }

  // printf("*****************[ u distribution ]******************\n");
  // for(int i=10; i<=20; i++){
  //     printf("[Iteration: %d] [User num: %ld]\n", i-10,
  //     user_distribution(user_generator));
  // }
}
...
std::vector<uint64_t>
SmallBankManager::get_next_transaction_by_type(uint64_t tx_type) {
  ...
  case 1: {
    ...
    auto amount =
        random_number_generator(0, (UINT64_MAX - checking_amount) / 2);
    tx_payload.push_back(amount);
  ...
  case 3: {
    /** void send_payment(uint64_t from_user_id, uint64_t to_user_id, uint64_t
     * amount) **/
    ...
    auto amount = random_number_generator(
        0, std::min<uint64_t>(from_checking_amount,
                              UINT64_MAX - to_checking_amount));
    tx_payload.push_back(amount);
  ...
  case 4: {
    /** void transaction_split(std::vector<std::pair<uint64_t, uint64_t>>
     * payors, std::vector<uint64_t> party) **/
    /** payload format : [tx type, count of payors, payer_1 id, payer_1 amount,
     * ..., party size, member_1, member_1, ...] **/

    /** Find the next party size users **/
    uint64_t party_size = random_number_generator(MIN_SPLIT_TX_PARTY_SIZE,
                                                  MAX_SPLIT_TX_PARTY_SIZE);
    ...
    for (uint64_t user_id = user_id_start; user_id < user_id_start + n_payors;
         user_id++) {
      max_amount = std::min(max_amount, bank->query(user_id).first);
    }
    for (uint64_t user_id = user_id_start + n_payors;
         user_id < user_id_start + party_size; user_id++) {
      max_amount =
          std::min(max_amount, UINT64_MAX - bank->query(user_id).first);
    }
    ...
    for (uint64_t user_id = user_id_start; user_id < user_id_start + party_size;
         user_id++) {
      tx_payload.push_back(user_id);
    }
...
SmallBankManager::execute_transaction(const uint64_t *tx_payload) {
...
std::pair<uint64_t, uint64_t>
SmallBankManager::show_account_info(uint64_t user_id) {
...
//     // bank->amalgamate(5);
//     // std::vector<std::pair<uint64_t,uint64_t>> payors{std::make_pair(6,30),
//     std::make_pair(7,20)};
...
//     // for(uint64_t i=0; i<n; i++){
//     //     printf("[User: %ld] [C: %ld] [S: %ld]\n", i,
//     manager->show_account_info(i).first,
//     manager->show_account_info(i).second);
//     // }
...
//     //     printf("\n");
//     //     printf("[User: %ld] [C: %ld] [S: %ld]\n", payload[1],
//     manager->show_account_info(payload[1]).first,
//     manager->show_account_info(payload[1]).second);
//     // }
