...
class HotStuffCore {
     ...
     * functions. */
    void on_init(uint32_t nfaulty, double fairness_parameter, double gen_param, double network_param);  // FlashOrder-D
    ...
    block_t on_propose(const std::vector<uint256_t> &cmds,                      // FlashOrder: 线性排序结果
                    const std::vector<block_t> &parents,
                    bytearray_t &&extra = bytearray_t());

    /** Call to submit local order to the current leader **/
    void on_local_order (ReplicaID proposer, const std::vector<uint256_t> &order,
                         const std::vector<uint64_t> &timestamps);              // FlashOrder
    /** Called when local order is received on Leader from a Replica  **/
    bool on_receive_local_order (const LocalOrder &local_order, const std::vector<block_t> &parents);   // FlashOrder
    /** FlashOrder: 直接返回 block 中的 cmds（单轮完成，无需多轮 finalize） **/
    std::vector<uint256_t> fair_finalize(block_t const &blk);                   // FlashOrder

    /** FlashOrder: 执行一轮排序，收集所有副本的 OrderedList 并生成最终排序 **/
    std::vector<uint256_t> flashorder_propose();                                // FlashOrder

    /** FlashOrder Initialization **/
    double get_solid_tx_threshold();
...
struct LocalOrder: public Serializable {
    ...
    /** 每笔交易的本地接收时间戳（微秒），与 ordered_hashes 一一对应 **/
    ...
    void serialize(DataStream &s) const override {
        ...
        s << htole((uint32_t)timestamps.size());
    ...
    operator std::string () const {
        ...
        s << " timestamps=[" << std::to_string(timestamps.size()) << "]";
        s << ">";
        return s;  
    }
};
