...
class HotStuffBase: public HotStuffCore {
    ...
    std::queue<uint256_t> local_order_buffer;               // FlashOrder
    std::queue<uint64_t> timestamp_buffer;                  // FlashOrder: 对应每个交易的本地接收时间戳
    ...
    inline void local_order_handler(MsgLocalOrder &&, const Net::conn_t &);     // FlashOrder
     /** Called upon receiving local order from replicas to the Leader. */
    inline void process_local_order(const LocalOrder &);                        // FlashOrder

    inline bool conn_handler(const salticidae::ConnPool::conn_t &, bool);

    void do_broadcast_proposal(const Proposal &) override;
    void do_vote(ReplicaID, const Vote &) override;
    void do_send_local_order(ReplicaID, const LocalOrder &) override;       // FlashOrder
...
class HotStuff: public HotStuffBase {
    ...
    void start(const std::vector<std::tuple<NetAddr, bytearray_t, bytearray_t>> &replicas, 
        ...
        HotStuffBase::start(std::move(reps), fairness_parameter, gen_param, network_param, ec_loop);      // FlashOrder-D
    }
};
