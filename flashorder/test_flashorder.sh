...
for i in {0..3}; do
    echo "  启动副本 $i..."
    ./build/examples/hotstuff-app --conf hotstuff-sec${i}.conf > log${i}.log 2>&1 &
    REPLICA_PIDS[$i]=$!
    sleep 0.5
done
...
echo "6. 启动客户端测试（10秒）..."
timeout 10 ./build/examples/hotstuff-client --idx 0 --iter 100 --max-async 10 > client_raw.log 2>&1
