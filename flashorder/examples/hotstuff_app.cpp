...
class HotStuffApp: public HotStuff {
    ...
    void state_machine_execute(const Finality &fin) override {
        reset_imp_timer();
        auto cmd = storage->find_cmd(fin.cmd_hash);
        if (cmd) {
            auto cmd_dummy = static_cast<const CommandDummy *>(cmd.get());
            HOTSTUFF_LOG_INFO("replicated %s cid=%u", std::string(fin).c_str(), cmd_dummy->get_cid());
        } else {
    ...
    HotStuffApp(uint64_t sb_n_users,
                ...
                double gen_param,              // FlashOrder-D
                double network_param,          // FlashOrder-D
                uint32_t blk_size,
                double stat_period,
                double impeach_timeout,
                ReplicaID idx,
...
int main(int argc, char **argv) {
    ...
    auto opt_gen_param = Config::OptValDouble::create(0.0);  // FlashOrder-D: 0=auto-detect
    auto opt_network_param = Config::OptValDouble::create(0.0);  // FlashOrder-D: 0=auto-detect
    ...
    config.add_opt("gen-param", opt_gen_param, Config::SET_VAL, 'g', "FlashOrder-D: transaction generation interval (ms)");  // FlashOrder-D
    config.add_opt("network-param", opt_network_param, Config::SET_VAL, 'n', "FlashOrder-D: network latency expectation (ms)");  // FlashOrder-D
    ...
    papp = new HotStuffApp(opt_sb_users->get(),
                        ...
                        opt_gen_param->get(),          // FlashOrder-D
                        opt_network_param->get(),      // FlashOrder-D
                        opt_blk_size->get(),
                        opt_stat_period->get(),
                        opt_imp_timeout->get(),
                        idx,
    ...
    papp->start(reps, opt_fairness_parameter->get(), opt_gen_param->get(), opt_network_param->get());  // FlashOrder-D
    elapsed.stop(true);
    return 0;
}
...
HotStuffApp::HotStuffApp(uint64_t sb_n_users,
    ...
    (void)gen_param;  // FlashOrder-D: unused in constructor, passed via start()
    (void)network_param;  // FlashOrder-D: unused in constructor, passed via start()
...
void HotStuffApp::start(const std::vector<std::tuple<NetAddr, bytearray_t, bytearray_t>> &reps, 
    ...
    HotStuff::start(reps, fairness_parameter, gen_param, network_param);      // FlashOrder-D
