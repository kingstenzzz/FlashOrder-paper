...
// FlashOrder: MsgLocalOrder 实现
const opcode_t MsgLocalOrder::opcode;
MsgLocalOrder::MsgLocalOrder(const LocalOrder &lo) { serialized << lo; }
        ...
        part_delivery_time_max(0)
{
    ...
    pn.reg_handler(salticidae::generic_bind(&HotStuffBase::local_order_handler, this, _1, _2));
...
// FlashOrder: 发送 LocalOrder 给指定的 Leader
void HotStuffBase::do_send_local_order(ReplicaID proposer, const LocalOrder &local_order) {
    HOTSTUFF_LOG_DEBUG("[[do_send_local_order]] [R-%d] [L-%d] Sending local order", get_id(), proposer);
    if (proposer == get_id())
    {
        // 如果自己是 Leader，直接处理
        process_local_order(local_order);
...
// FlashOrder: 处理收到的 LocalOrder
void HotStuffBase::process_local_order(const LocalOrder &local_order) {
    HOTSTUFF_LOG_DEBUG("[[process_local_order]] [R-%d] Processing local order from R-%d", get_id(), local_order.initiator);
    auto proposer = pmaker->get_proposer();
    // 调用 on_receive_local_order 存储 OrderedList
    bool has_majority = on_receive_local_order(local_order, pmaker->get_parents());
    if (has_majority && proposer == get_id()) {
        // 收到足够多的 LocalOrder，触发 flashorder_propose
        HOTSTUFF_LOG_INFO("[[process_local_order]] [R-%d] Majority reached, calling flashorder_propose", get_id());
        auto cmds = flashorder_propose();
        if (!cmds.empty()) {
...
void HotStuffBase::start(
    ...
    on_init(nfaulty, fairness_parameter, gen_param, network_param);
    pmaker->init(this);
    if (ec_loop)
        ec.dispatch();
    ...
    cmd_pending.reg_handler(ec, [this](cmd_queue_t &q) {
        ...
        while (q.try_dequeue(e))
        {
            ...
            // FlashOrder: 每个副本都记录本地顺序和时间戳
            local_order_buffer.push(cmd_hash);
            timestamp_buffer.push(std::chrono::duration_cast<std::chrono::microseconds>(
                std::chrono::steady_clock::now().time_since_epoch()).count());
            
            HOTSTUFF_LOG_INFO("[[cmd_pending]] [R-%d] buffer_size=%lu", get_id(), local_order_buffer.size());
            
            if (local_order_buffer.size() >= blk_size)
            {
                HOTSTUFF_LOG_INFO("[[cmd_pending]] [R-%d] blk_size reached, calling on_local_order", get_id());
                ...
                for (uint32_t i = 0; i < blk_size; i++)
                {
                    ...
                    tss.push_back(timestamp_buffer.front());
                    timestamp_buffer.pop();
                }
                on_local_order(proposer, cmds, tss);
                return true;
            }
        }
        return false;
